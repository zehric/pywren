...
OUTPUT_NAME = "{}_{}_{}".format(args.data_size, NUM_KEYS, args.run_num)
...
def write_keys(prefix, size=DATA_SIZE, num_keys=10):
    ...
    fio.put_objects_async(ptrs_in, buffer_sizes, buckets, keys, d_ptr_s, d_ptr_f)
    e = time.time()
    return list(zip(doubles_s, doubles_f))

def read_keys(prefix, size=DATA_SIZE, num_keys=10):
    client = boto3.client('s3')
    fio = FastIO(so_bucket="zehric-pywren-149", prefix=prefix)
    fio.cache_so()
    fio.start_api()
    mat_in = np.zeros(int(size), np.uint8)
    doubles_s = np.zeros(num_keys, np.float64)
    doubles_f = np.zeros(num_keys, np.float64)
    t = time.time()
    ptrs_in = []
    ...
    d_ptr_s = doubles_s.ctypes.data_as(ctypes.POINTER(ctypes.c_double))
    d_ptr_f = doubles_f.ctypes.data_as(ctypes.POINTER(ctypes.c_double))
    fio.get_objects_async(ptrs_in, buffer_sizes, buckets, keys, d_ptr_s, d_ptr_f)
    e = time.time()
    return list(zip(doubles_s, doubles_f))
...
def profile_iops(results):
    ...
    bins = np.linspace(min_time, max_time, (max_time - min_time) * 100)
