...
trial_id="large_key_random_trials"
...
do
    for index in ${!num_cores[*]}; do                                               
        ...
        for exi in "${experiments[@]}"                                              
        do                                                                          
            python3 broadcast_benchmark_script.py launch $DATA_SIZE $NUM_KEYS $n_ii $n_ci $NUM_KEYS_PER_CORE $exi True False $trial_id 0
            sleep 150
            for r_fi in "${run_func[@]}"                                            
            do                                                                      
                #echo $r_fi $DATA_SIZE $NUM_KEYS $n_ii $n_ci $NUM_KEYS_PER_CORE $exi $launch_group $terminate_after
                python3 broadcast_benchmark_script.py $r_fi $DATA_SIZE $NUM_KEYS $n_ii $n_ci $NUM_KEYS_PER_CORE $exi $launch_group $terminate_after $trial_id $trial
                if [ "$launch_group" == "True" ]; then                              
                    launch_group=False                                              
                fi                                                                  
                total_to_launch="$(($total_to_launch-1))"                           
